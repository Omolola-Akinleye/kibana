...
export default function (providerContext: FtrProviderContext) {
  ...
  describe('fleet_cloud_connectors', () => {
    ...
    describe('POST /api/fleet/cloud_connectors', () => {
      ...
      it('should handle AWS role_arn with aws.role_arn format', async () => {
        const { body } = await supertest
          .post(`/api/fleet/cloud_connectors`)
          .set('kbn-xsrf', 'xxxx')
          .send({
      ...
      it('should accept valid external_id with numbers only', async () => {
        const { body } = await supertest
          .post(`/api/fleet/cloud_connectors`)
          .set('kbn-xsrf', 'xxxx')
          .send({
            name: 'test-aws-connector-numbers',
            cloudProvider: 'aws',
            vars: {
              role_arn: 'arn:aws:iam::123456789012:role/test-role',
              external_id: {
        ...
        expect(body).to.have.property('id');
        expect(body.name).to.equal('test-aws-connector-numbers');
        expect(body.vars.external_id).to.have.property('id', '12345678901234567890');
      });
